...
void CRoute::addPoi(string namePoi, string afterWp){
	...
	for(list<CWaypoint*>::reverse_iterator itr = this->m_route.rbegin(); itr != this->m_route.rend(); ++itr){
		...
		temp = dynamic_cast<CPOI*>(*itr);

		if((*itr)->getName() == afterWp && temp == NULL){
...
void CRoute::print(){
	...
	if(!this->m_route.empty())
		...
		throw runtime_error("No Route to print");
...
CRoute& CRoute::operator +=(const string name){
	...
	if(this->m_pPoiDatabase == NULL || this->m_pWpDatabase == NULL){
		//cout << "ERROR: not connected to database or database is empty"<<endl;
		return *this;
	}
...
const vector<const CWaypoint*> CRoute::getRoute(){
	...
	vector<const CWaypoint*> vroute;

	for(list<CWaypoint*>::const_iterator itr = m_route.begin(); itr!=m_route.end();++itr){
		CWaypoint* c = &(**itr);
		vroute.push_back(const_cast<const CWaypoint*>(c));
	}
